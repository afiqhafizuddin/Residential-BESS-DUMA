...
DSSText.command='Compile C:\Uniten_RCT\Research\Research_Supervision\Masters\Masters_project_Dilini\Software\TNB_network\Storage_RBESS\masters_BESS_Bat_120.dss';
...
DSSText.Command='Redirect RBESS_120.dss'
    ...
    if P_Gen_extract(h+1,1)>0
        if Excess_P_Gen(h+1)>0
            if h>=C1-1 && h<=C2-1
                ...
                for k=1:RB
                    if Voltagecus(k,h)>=1.099
                        P_expo_lim(h+1,k)=((1.099)/Voltagecus(k,h))*(Excess_P_Gen(h,k)-R_charge(h,k)*(BESS_kW/100));
                    end
...
ylabel('Power (kW)')
legend('w/o RBESS', 'RBESS w/o VC','RBESS w VC','FontSize',8);
title('Variation of net power at the critical node');
...
Mean_D1=[mean(GUI_WOB_D1(:,1)) mean(GUI_WB_WVC_D1(:,1)) mean(GUI_WB_WoVC_D1(:,1))]
Mean_D2=[mean(D11) mean(D12) mean(D13)]
figure()
subplot(121)
bar(Mean_D1)
ylim([0 2.5])
...
GUID1WOB=(sum(GUI_WOB_D1(:,1))/sum(P_L_extract(1:1440,1)))*100
GUID1WB=(sum(GUI_WB_WVC_D1(:,1))/sum(P_L_extract(1:1440,1)))*100

GUID2WOB=(sum(GUI_WOB_D2(:,1))/sum(P_L_extract(1441:2880,1)))*100
GUID2WB=(sum(GUI_WB_WVC_D2(:,1))/sum(P_L_extract(1441:2880,1)))*100
